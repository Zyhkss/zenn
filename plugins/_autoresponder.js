...
let loveyou = lovou[Math.floor(Math.random() * lovou.length)]
    ...
    let helloZyhkss = fs.readFileSync('/PTT-20211218-WA0243.opus') 
    conn.sendMessage(m.chat, {
        react: {
          text: `${loveyou}`,
          key: m.key,
   ...
   setTimeout(() => {
        conn.sendFile(m.chat, helloZyhkss, '', '', m, true)
    }, 1000)
    }
...
let assl = assal[Math.floor(Math.random() * assal.length)]
    if (isAss && !m.fromMe) {
    let assalamualaikum = fs.readFileSync('/WhatsApp-Audio-2021-03-02-at-20.46.15.opus')
    conn.sendMessage(m.chat, {
        react: {
          text: `${assl}`,
          key: m.key,
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
   ...
   /* let hiai = /(hi|hii|hiii|hi Zyhkss|hii Zyhkss|hiii Zyhkss|hy|halo|hallo|helo|hello|hy Zyhkss|halo Zyhkss|hallo Zyhkss|helo Zyhkss|hello Zyhkss)/i
...
let hiZyhkss = hiain[Math.floor(Math.random() * hiain.length)]
    if (isAih && !m.fromMe) {
    let helloZyhkss = fs.readFileSync('/WhatsApp-Ptt-2021-07-14-at-18.12.33.opus') 
    conn.sendMessage(m.chat, {
        react: {
          text: `${hiZyhkss}`,
          key: m.key,
